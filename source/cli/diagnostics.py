...
def _collect_parser_diagnostics(source_file: str) -> list[Diagnostic]:
    """Return the parser failure that would otherwise appear only at compile time."""
    try:
        _parse_program_ast(source_file, resolve_imports=True)
        return []
    except (OSError, ValueError, SyntaxError) as error:
        message = str(error)
        match = re.match(r"Line (\d+),?\s*Col (\d+):?\s*(.*)", message)
        if match:
            return [
                Diagnostic(
                    int(match.group(1)),
        ...
        return [
            Diagnostic(1, 1, message or "source could not be parsed", severity="error")
        ]
...
def run_diagnostics(source_file: str, fix_mode: bool = False) -> int:
    ...
    try:
        diagnostics = analyze_file(source_file)
        diagnostics.extend(_collect_parser_diagnostics(source_file))
        ...
        if fix_mode:
            ...
            remaining.extend(_collect_parser_diagnostics(source_file))
