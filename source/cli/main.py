...
def main():
    ...
    if effect_policy_json:
        # Keep JSON output machine-parseable by skipping prepass chatter.
        skip_prepass = True
    # --profile: instrument every AILang function with entry/exit hooks
    # and print a systemd-analyze blame-style report after the JIT run.
    # Adds noticeable overhead, so it's strictly opt-in.
    profile_enabled = "--profile" in sys.argv
    # --profile-flame=path: also write folded-stack data for flamegraph.pl.
    # Implies --profile (the data comes from the same instrumentation).
    flame_path = ""
    for arg in sys.argv:
