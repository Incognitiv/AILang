...
class ContextEmitter:
    ...
    def register_bigint_for_cleanup(self, var_name: str, value_or_slot: Any) -> None:
        """Register an owning ``unbounded`` value for scope cleanup."""
        if self._cg.scope_cleanup_stack:
            self._cg.scope_cleanup_stack[-1].append(
                (var_name, "__ailang_bigint__", value_or_slot)
            )
