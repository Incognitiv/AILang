...
def _build_jit_callable(
    ...
    try:
        ...
        raw_main = ctypes.CFUNCTYPE(
            ctypes.c_int,
            ctypes.c_int,
            ctypes.POINTER(ctypes.c_char_p),
        )(func_addr)

        # Native AILang executables enter through the platform C ABI
        # `int main(int argc, char **argv)`.  JIT must honor the same ABI;
        # calling the address as a zero-argument function leaves argc/argv
        # undefined and makes argc()/argv() observably diverge from AOT.
        argv0 = os.fsencode(source_file or "ailang-jit")
        argv_storage = (ctypes.c_char_p * 2)()
        argv_storage[0] = argv0
        argv_storage[1] = None

        def cfunc() -> int:
            return int(raw_main(1, argv_storage))
