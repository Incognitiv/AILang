...
Example:
    Generic: record Pair[T, U]: T first; U second end
    Usage:   Pair[int, string] p = new Pair[int, string](1, "hello")
    Creates: record Pair_int_string: int first; string second end
"""
...
def monomorphize_record(
    ...
    Example:
        Generic: record Pair[T, U]: T first; U second end
        Args:    [int, string]
        Result:  record Pair_int_string: int first; string second end
    """
    ...
    # Substitute types in fields. RecordDef stores fields as
    # ``(field_name, field_type)``; keep that convention here as well.
    ...
    defaults = dict(getattr(generic_record, "field_defaults", {}) or {})
    return A.RecordDef(mangled_name, new_fields, defaults)
