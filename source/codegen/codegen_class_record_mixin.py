...
class _CodeGenClassRecordMixin:
    ...
    def generate_record(self: Any, node: Any) -> None:
        ...
        self.record_field_defaults[node.name] = dict(
            getattr(node, "field_defaults", {}) or {}
        )
    ...
    def _generate_method(
        ...
        # Reuse the same typed-return path as free functions.  The statement
        # emitter resolves the source-level return specification through this
        # table; without a method entry it only sees the LLVM integer width and
        # can lose signedness/range semantics.
        self._function_nodes[mangled_name] = method
