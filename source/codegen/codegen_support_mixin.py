...
class _CodeGenSupportMixin:
    ...
    def _print_bigint_dec(self: Any, value: ir.Value, _is_unsigned: bool) -> None:
        ...
        # Working value and constants. Signed values are converted to an
        # unsigned magnitude in the same bit width; modulo subtraction keeps
        # the minimum representable value well-defined.
        zero = ir.Constant(value.type, 0)
        ten = ir.Constant(value.type, 10)
        if _is_unsigned:
            is_negative = ir.Constant(ir.IntType(1), 0)
            val = value
        else:
            is_negative = self.current_builder.icmp_signed(
                "<", value, zero, name="bigint_print_neg"
            )
            neg_mag = self.current_builder.sub(zero, value, name="bigint_print_neg_mag")
            val = self.current_builder.select(
                is_negative, neg_mag, value, name="bigint_print_mag"
            )
        ...
        # Print sign + digits without narrowing the value.
        fmt = self.create_string_constant("%s%.*s\n")
        minus = self.create_string_constant("-")
        empty = self.create_string_constant("")
        sign = self.current_builder.select(
            is_negative, minus, empty, name="bigint_print_sign"
        )
        len_i32 = final_len
        buf_ptr = self.current_builder.gep(
            rev_buf, [ir.Constant(ir.IntType(32), 0), ir.Constant(ir.IntType(32), 0)]
        )
        self.current_builder.call(printf, [fmt, sign, len_i32, buf_ptr])
