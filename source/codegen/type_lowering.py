    ...
    def get_llvm_type(self, type_spec: Any) -> Any:
        ...
        if isinstance(type_spec, str):
            ...
            if type_lower.startswith("f"):
                ...
                if type_lower == "f128":
                    raise TypeError(
                        "LLVM backend: 'f128/quad' requires real IEEE binary128; "
                        "llvmlite cannot lower fp128, refusing silent f64 fallback"
                    )
            ...
            if type_lower == "unbounded":
                # True arbitrary-precision integer.  Storage is an owning
                # pointer to the shared native sign+limb runtime object; no
                # fixed-width LLVM integer is substituted here.
