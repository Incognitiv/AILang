...
def emit_safety_helpers(self) -> None:
    ...
    # Stack overflow protection - always emit
    self._output.append("/* C23 Safety: Stack overflow protection */")
    self._output.append("static int64_t __ailang_recursion_depth = 0;")
    self._output.append("#define AILANG_MAX_RECURSION_DEPTH 10000")
    self._output.append("")
    self._output.append(
    ...
    # Command-line arguments - always emit for compatibility
    if "cmdline" in self._needs.helpers:
        ...
        self._output.append("}")
        self._output.append("")
        # getenv: thin wrapper around libc, returning "" instead of
        # NULL so user code can `strlen(getenv("X"))` etc. without
        # null-checking. The pointer is libc-owned (borrowed) -- do
        # NOT free it; safe by design (not in _STRING_OWNING_CALLS).
