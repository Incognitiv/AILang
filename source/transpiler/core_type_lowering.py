...
class _CTranspilerTypeLoweringMixin:
    ...
    def _ailang_type_to_c(self: Any, atype: str) -> str:
        ...
        if type_lower == "unbounded":
            # Backend representation only: language-level arbitrary-precision
            # semantics live in stdlib/core/bigint.ail.  The C backend carries
            # the owning object as an opaque pointer and compiles that same .ail
            # implementation into the generated translation unit.
            return "void *"

        if type_lower in {"quad", "f128"}:
            raise ValueError(
                "C backend: 'f128/quad' requires real IEEE binary128; "
                "long double is target-dependent and is not accepted as f128"
            )
        ...
        type_map = {
            ...
            # 256-bit and above: C23 native extended integers.
            # Both Clang and GCC expose standard C23 _BitInt; aliases are
            # emitted only when a program actually uses the wide ladder.
            "wide": "ailang_i256",
            "i256": "ailang_i256",
            "uwide": "ailang_u256",
            "u256": "ailang_u256",
