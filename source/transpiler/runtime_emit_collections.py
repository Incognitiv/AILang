...
def emit_runtime_dict(self) -> None:
    ...
    self._output.append(
        "    d->entries = (ailang_dict_entry*)calloc("
    ...
    self._output.append("        }")
    self._output.append("        free(old);")
    self._output.append("    }")
    self._output.append("    uint64_t idx = dict_hash(key) % (uint64_t)d->capacity;")
    self._output.append("    while (d->entries[idx].occupied) {")
    self._output.append("        if (strcmp(d->entries[idx].key, key) == 0) {")
    ...
    self._output.append("    int64_t start = (int64_t)idx;")
    self._output.append("    while (d->entries[idx].occupied) {")
    self._output.append("        if (strcmp(d->entries[idx].key, key) == 0) return 1;")
    self._output.append("        idx = (idx + 1) % (uint64_t)d->capacity;")
    self._output.append("        if ((int64_t)idx == start) break;")
    self._output.append("    }")
    ...
    self._output.append("    free(d->entries);")
    self._output.append("    ailang_safe_free(d);")
    self._output.append("}")
    self._output.append("")
...
def emit_runtime_str_array(self) -> None:
    ...
    self._output.append("    }")
    self._output.append("    size_t sep_len = sep ? strlen(sep) : 0;")
    self._output.append("    size_t total = 0;")
    self._output.append("    for (int64_t i = 0; i < arr.length; i++) {")
    self._output.append("        if (arr.data[i]) total += strlen(arr.data[i]);")
    self._output.append("    }")
    self._output.append(
    ...
    self._output.append("            memcpy(p, sep, sep_len);")
    self._output.append("            p += sep_len;")
    self._output.append("        }")
    self._output.append("        if (arr.data[i]) {")
    self._output.append("            size_t len = strlen(arr.data[i]);")
    self._output.append("            memcpy(p, arr.data[i], len);")
