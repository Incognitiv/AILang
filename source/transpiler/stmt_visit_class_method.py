    ...
    def walk(items) -> None:
        for stmt in items or []:
            if isinstance(stmt, A.VarDecl) and stmt.type_name is not None:
                if parsed_type_to_str(stmt.type_name).strip().lower() == "unbounded":
...
def _prepare_owned_local_cleanup(
    ...
    bigint_vars = _scan_unbounded_locals(body)
    all_var_names = (
    ...
    # BigInts are immutable owning values. Even when returned, transfer is handled
    # by visit_Return exclusion; ordinary locals are otherwise always cleaned.
    self._bigint_locals_for_cleanup = list(bigint_vars)
...
def _emit_owned_local_initializers(
    ...
    for var in non_escaping_lc_str_array:
        mangled = self._mangle_var(var)
        self.emit(f"{mangled} = (typeof({mangled})){{0}};")
    for var in getattr(self, "_bigint_locals_for_cleanup", None) or []:
        self.emit(f"{self._mangle_var(var)} = NULL;")
...
def _generate_class_method(self, class_name: str, method: A.Function) -> None:
    ...
    # visit_Return needs both the lowered C type and the original AILang
    # declaration.  Free functions already establish this context; class
    # methods historically did not, which let e.g. i8(-1) silently become
    # u8(255) at a method return boundary.
    self._current_ret_type = ret_type
    self._current_ret_spec = getattr(method, "return_type", None)
    ...
    if not method.body or not isinstance(method.body[-1], A.Return):
        ...
        if ret_type == "void":
            ...
            raise ValueError(
                f"non-void method {class_name}.{method_name} can reach end without returning a value"
            )
