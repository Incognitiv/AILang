...
def _builtin_syscall(self, args) -> ir.Value:
    ...
    is_linux = "linux" in triple or (not triple and sys.platform.startswith("linux"))
    if not is_linux:
    ...
    is_x86_64 = arch in {"x86_64", "amd64"} or (
        not arch and platform.machine().lower() in {"x86_64", "amd64"}
    )
    if is_x86_64:
        # Linux x86-64 syscall ABI:
        #   rax = number, rdi/rsi/rdx/r10/r8/r9 = arguments, rax = result.
        # Pad absent arguments so the inline-asm signature and constraints stay
        # fixed for every source-level syscall arity.
        values.extend(ir.Constant(i64, 0) for _ in range(7 - len(values)))
        asm_ty = ir.FunctionType(i64, [i64] * 7)
        asm = ir.InlineAsm(
            asm_ty,
            "syscall",
        ...
        return self.builder.call(asm, values, name="syscall_result")
