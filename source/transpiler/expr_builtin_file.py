...
class ExprBuiltinFileEmitter:
    ...
    def _builtin_write_file(self, args):
        ...
        self.builder.position_at_end(success_block)
        # Set 64KB buffer for performance (_IOFBF = 0 for full buffering)
        null_buf = ir.Constant(ir.IntType(8).as_pointer(), None)
        iofbf = ir.Constant(ir.IntType(32), 0)  # _IOFBF = full buffering
        buf_size = ir.Constant(ir.IntType(64), 65536)  # 64KB
        self.builder.call(
            self.codegen.get_setvbuf(), [file_ptr, null_buf, iofbf, buf_size]
        )
