...
from .llvm_fixed_int_casts import cast_to_declared_int
...
def _try_emit_stack_class_vardecl(self, node: VarDecl, llvm_type: ir.Type) -> bool:
    ...
    # Deterministic zero initialization first; declaration defaults are
    # applied in a second pass so hidden string-length fields are not
    # accidentally overwritten by the generic zero loop.
    ...
    for field_name, default_expr in class_defaults.items():
        field_idx, field_type = self.codegen.get_field_info(class_name, field_name)
        field_llvm_type = self.codegen.get_llvm_type(field_type)
        default_val = self.codegen.generate_expr(default_expr)
        ...
        field_ptr = self.builder.gep(
            ...
            name=f"field_{field_name}_default_ptr",
        )
        self.builder.store(default_val, field_ptr)
    ...
    if constructor_name in self.codegen.functions:
        ...
        source_args = list(node.init_value.args)
        if len(source_args) > len(visible_fields):
            raise StmtGenError(
                f"{class_name} expects at most {len(visible_fields)} constructor arguments, "
                f"got {len(source_args)}"
            )
        for field_info in visible_fields[len(source_args) :]:
            field_name = str(field_info[0])
            default_expr = class_defaults.get(field_name)
            if default_expr is None:
                raise StmtGenError(
                    f"{class_name} requires constructor argument for field '{field_name}'"
                )
            source_args.append(default_expr)
        for index, ((_, field_type), arg_expr) in enumerate(
            zip(visible_fields, source_args, strict=False)
        ):
            field_name = visible_fields[index][0]
            field_llvm_type = self.codegen.get_llvm_type(field_type)
            arg_value = self.codegen.generate_expr(arg_expr)
            arg_value = cast_to_declared_int(
                self.codegen, arg_value, field_llvm_type, field_type
            )
