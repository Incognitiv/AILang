...
class ExprLiteralEmitter:
    ...
    def visit_Number(self, node: Number) -> ir.Constant:
        ...
        if bitlen > candidate_widths[-1]:
            raise ExprGenError(
                "integer literal exceeds the fixed i8192/u8192 ladder; "
                "use it in an unbounded context instead of truncating it"
            )
    ...
    def visit_ReinterpretCast(self, node) -> ir.Value:
        ...
        # Int to int (bit-pattern preserving truncate/zero-extend).  A
        # reinterpret cast also establishes the target language signedness;
        # LLVM integer types themselves do not carry that information.
        if isinstance(src_type, ir.IntType) and isinstance(target_llvm, ir.IntType):
            if src_type.width > target_llvm.width:
                out = self.builder.trunc(value, target_llvm, name="reinterp")
            elif src_type.width < target_llvm.width:
                out = self.builder.zext(value, target_llvm, name="reinterp")
            else:
                out = value
            info = info_for_fixed_int(node.target_type)
            if info is not None:
                self.codegen.set_signedness(out, not info.unsigned)
            return out
