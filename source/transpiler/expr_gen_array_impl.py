...
def _expr_array_access(self, node: A.ArrayAccess) -> str:
    ...
    if isinstance(node.array, A.Variable) and hasattr(self, "_var_types"):
        ...
        if isinstance(vtype, str) and (
            ...
            self.used_helpers.add("safe_array")
            return f"ailang_safe_array_get({arr}.data, {idx}, {arr}.length)"
    # Fixed-size array aliases lower to native C arrays (e.g. int64_t arr[8]),
    # not to the ailang_array {data,length} runtime struct. `_array_vars` also
    # contains these locals, so recognize the declared fixed-array type first.
    if isinstance(node.array, A.Variable) and hasattr(self, "_var_types"):
        vtype = self._var_types.get(node.array.name, "")
        parsed = None
        ...
        if parsed is not None:
            ...
            self.used_helpers.add("safe_array")
            return f"ailang_safe_array_get({arr}, {idx}, {int(fixed_len)})"
...
def _known_array_len_hint(self, array_expr: A.ASTNode) -> int | None:
    ...
    if hasattr(self, "_var_types"):
        ...
        if isinstance(atype, str) and hasattr(self, "_parse_fixed_array_type_spec"):
            resolved_atype = self._resolve_type_alias_spec(atype)
            parsed = self._parse_fixed_array_type_spec(resolved_atype)
            if parsed is not None:
