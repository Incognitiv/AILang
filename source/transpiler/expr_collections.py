...
class ExprCollectionEmitter:
    ...
    def visit_ArrayAccess(self, node: ArrayAccess):
        ...
        index_val = self.ensure_int64(key_val)
        ...
        if (
            ...
            # Skip bounds check if unsafe flag is set (user approved)
            if not getattr(node, "unsafe", False) and not int_literal_in_range(
                node.index, 0, array_len
            ):
