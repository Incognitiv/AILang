...
def observe_calls_in_expr(
    ...
    for node in analyzer._walk_ast(expr):
        ...
        for idx, arg in enumerate(node.args or []):
            ...
            info = _string_info_from_expr(facts, func_scope, arg)
            if info is not None:
                facts.observe_call_arg_string_info(node.name, idx, info)


def _string_info_from_expr(facts: Any, func_scope: Optional[str], expr: A.ASTNode):
    ...
    if isinstance(expr, A.Cast):
        return _string_info_from_expr(facts, func_scope, expr.expr)
    return None
...
def _strlen_source_var(expr: Optional[A.ASTNode]) -> Optional[str]:
    if not isinstance(expr, A.Call):
        return None
    if expr.name != "strlen" or len(expr.args or []) != 1:
        return None
    arg = arg_at(expr, 0)
...
def _remember_or_clear_string_info(
    ...
    if expr is None:
        return
    info = _string_info_from_expr(facts, func_scope, expr)
    if info is not None:
...
def _refine_scope_for_condition(
    ...
    *,
    ...
    # Keep this refinement narrow for now: it is intended to prove byte/char
    # parser guards (`d >= '0'`, `d <= '9'`) without changing broad
    # branch-heavy loop conservatism.
    if current is None or current.low < 0 or current.high != 255:
        return
