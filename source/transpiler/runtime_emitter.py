...
class RuntimeEmitter:
    ...
    def run(self) -> None:
        ...
        emit_runtime_fixed_int_casts(self)
        if self._needs.wide_ints:
            emit_runtime_wideint(self)
        ...
        if "bigint" in helpers:
            self._emit_runtime_bigint()
        if "simd" in helpers:
    ...
    def _emit_runtime_bigint(self) -> None:
        """No foreign BigInt runtime.

        `unbounded` semantics are compiled from ``stdlib/core/bigint.ail`` by
        the normal import pipeline.  This hook intentionally emits nothing; it
        remains only so runtime-needs reporting can preserve the helper family.
        """
        return
