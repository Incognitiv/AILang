...
def _get_printf_spec(self, arg_node: A.ASTNode) -> str:
    ...
    if inferred == "bool":
        return "@bool"
    wide = info_for_c(inferred)
    if wide is not None:
        return "@wide:" + wide.suffix
...
def _get_printf_arg(self, spec: str, c_expr: str, arg_node: A.ASTNode) -> str:
    ...
    if spec == "@bool":
        return c_expr
    if spec.startswith("@wide:") or spec.startswith("@fixed:"):
        return c_expr
    ...
    if direct_supported and interpolation_direct_supported:
        ...
        for i, spec in enumerate(specs):
            ...
            if spec == "%s":
                ...
                if expression_is_borrowed_bigint(arg_node):
                    self.emit(f"        ailang_bigint_print({expr_args[i]});")
                else:
                    tmp = f"__pr_bigint_{i}"
                    self.emit(f"        void *{tmp} = (void *)({expr_args[i]});")
                    self.emit(f"        ailang_bigint_print({tmp});")
                    self.emit(f"        ailang_bigint_free({tmp});")
            elif spec.startswith("@wide:"):
...
def _emit_method_call_text(self, node: A.MethodCall) -> str:
    ...
    # Keep method calls behind their real typed ABI during the correctness
    # phase. The old trivial-method inline path could skip return conversion
    # traps (for example i8(-1) returned from a u8 method).
    args = []
    ...
    for index, arg in enumerate(node.args):
        ...
        if index < len(params) and not can_elide_virtual:
            param = params[index]
            if isinstance(param, tuple) and len(param) >= 2:
                checked_arg = checked_fixed_int_conversion_expr(
                    self, arg, arg_expr, param[1]
                )
