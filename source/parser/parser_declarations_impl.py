...
def parse_function(
    ...
    if is_type_prefix:
        ...
        if is_type_token and not is_var_decl_pattern:
            ...
            # Temporary placeholder.  A whole-program return-type inference pass
            # resolves unannotated ``def name(...):`` functions after parsing.
            return_type = "i64"
    ...
    plain_fn.return_type_explicit = return_type_explicit
...
def parse_record(self) -> RecordDef | GenericRecord:
    ...
    field_defaults: dict[str, ASTNode] = {}
    saw_default = False
    while self.peek() and self.peek_type() != "END":
        ...
        if self.peek_type() == "ASSIGN":
            self.consume("ASSIGN")
            field_defaults[field_name] = self.parse_expression()
            saw_default = True
        elif saw_default:
            self.error(
                "Record fields without defaults cannot follow fields with defaults"
            )
    ...
    if type_params:
        return A.GenericRecord(name, type_params, fields, field_defaults)
    return RecordDef(name, fields, field_defaults)
