...
def _parse_global_var(self) -> VarDecl:
    ...
    # Fixed-width globals must have a compile-time integer initializer for now.
    # LLVM used to silently replace unsupported expressions with zero while the
    ...
    if match is not None:
        ...
        if isinstance(init_value, Number) and not init_value.is_float:
            literal_value = int(init_value.value)
        elif (
            isinstance(init_value, UnaryOp)
            and init_value.op in {"+", "-"}
            and isinstance(init_value.operand, Number)
            and not init_value.operand.is_float
        ):
            literal_value = int(init_value.operand.value)
            if init_value.op == "-":
                literal_value = -literal_value
        if literal_value is None:
            raise SyntaxError(
                f"Global fixed-width integer '{var_name}' requires a compile-time "
                "integer literal initializer; runtime global initialization is not implemented"
            )
        ...
        if not (low <= literal_value <= high):
            raise SyntaxError(
                f"Global initializer {literal_value} does not fit {type_text}"
            )
