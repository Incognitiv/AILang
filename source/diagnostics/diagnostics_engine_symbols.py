...
def _is_ident_type_name(value: str) -> bool:
    return value in (TYPE_NAMES | VECTOR_TYPE_NAMES) or bool(
        re.fullmatch(r"[iuf]\d+", value)
    )
...
def _collect_symbols_from_file(self, filepath: str, visited: set | None = None) -> None:
    ...
    try:
        ...
        while i < len(tok_list):
            ...
            imported_type_prefix = ttype in TYPE_PREFIX_TOKENS or (
                ttype == "IDENT" and _is_ident_type_name(tok_list[i][1])
            )
            if (imported_type_prefix and i + 2 < len(tok_list)) and (
            ...
            if tok_list[i][0] == "IMPORT":
                ...
                if sub_parts:
                    ...
                    sub_candidates = [
                        ...
                        os.path.join(imported_dir, "..", "..", "..", sub_rel),
                        os.path.join(imported_dir, "..", "..", "..", "..", sub_rel),
                        os.path.join(repo_root, sub_rel),
