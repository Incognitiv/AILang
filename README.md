...
The canonical declaration form puts the return type before the function name.
Legacy `def` declarations remain accepted for source compatibility while the
repository migrates to the single type-first surface.
...
Blocks are delimited explicitly: declarations use `: ... end`, while legacy/control-flow constructs may still use `then ... end`. Newlines separate statements where the grammar expects
...
Parameters may use either `name: type` or `type name`. Source-level `quad` parses
as `f128`, but code generation currently fails closed: neither backend may silently
substitute `double`/`long double` for real IEEE binary128. `void main():` is an
AILang no-return/no-argv entrypoint; native C output still uses
