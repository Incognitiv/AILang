...
def test_c_backend_fixed_array_local_indexing_runs(tmp_path: Path) -> None:
    src = tmp_path / "c_fixed_array_index.ail"
    out_stem = tmp_path / "c_fixed_array_index"
    src.write_text(
        """\
type Arr8 = [int; 8]

def sum_arr(iterations: int): int
