...
def test_llvm_argv_byte_option_check_keeps_prebranch_constant_state(
    tmp_path: Path,
) -> None:
    src = tmp_path / "argv_option_branch.ail"
    src.write_text(
        """\
int main():
    int parse_options = 1
    string argument = argv(1)
    int argument_data = reinterpret(int, argument)

    if strlen(argument) == 2 and peek8(argument_data, 0) == 45 and peek8(argument_data, 1) == 45 then
        parse_options = 0
    else
        if parse_options != 0 and strlen(argument) > 1 and peek8(argument_data, 0) == 45 then
            return 0
        end
    end
...
""",
    ...
    out = tmp_path / "argv_option_branch_llvm"
    proc = subprocess.run(
        [
            sys.executable,
            str(AILANG),
