...
def test_c_backend_ord_accepts_char_code_from_char_at() -> None:
    source = """\
def main(): int
    s = "abc"
    c = char_at(s, 1)
...
"""
    c_text = _transpile_c(source)
    main_body = c_text.rsplit("int main", 1)[1]
    assert "char_at(s, 1LL, -1LL)" in main_body
    assert "char_at(s, 1LL, -1LL))[0]" not in main_body
    assert "(c)[0]" not in main_body


def test_c_backend_typeof_char_at_is_int() -> None:
    source = """\
def main(): int
    s = "abc"
    print(typeof(char_at(s, 0)))
    return 0
end
"""
    c_text = _transpile_c(source)
    main_body = c_text.rsplit("int main", 1)[1]
    assert '\"int\"' in main_body
