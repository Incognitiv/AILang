...
"""
    ...
    method_end = ir_text.index('define i32 @"main"')
...
"""
    ...
    assert "__ailang_i64_to_cstr" not in ir_text
    assert "str_lit_i64_buf" not in ir_text
    assert 'call void @"Packet_init"' in ir_text
...
"""
    ...
    # Correctness phase: method-call inlining is intentionally not required;
    # typed parameter/return boundaries must remain observable until the typed
    # optimizer is reintroduced.
